...
void TAMC_GT911::begin(uint8_t _addr) {
  addr = _addr;
  G911.begin(pinSda, pinScl,400000);
  reset();
}
...
bool  TAMC_GT911::touch_has_signal(){
  return true;
}

bool  TAMC_GT911::touch_touched()
{
  ...
  if (isTouched)
  {
    ...
    touch_last_x = map(points[0].y, TOUCH_MAP_X1, TOUCH_MAP_X2, 0,799);
    touch_last_y = map(points[0].x, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0, 479);
#else
    touch_last_x = map(points[0].x, TOUCH_MAP_X1, TOUCH_MAP_X2, 0, 799);
    touch_last_y = map(points[0].y, TOUCH_MAP_Y1, TOUCH_MAP_Y2, 0,479);
#endif
    return true;
  }
  else
...
bool TAMC_GT911::touch_released(){
  return true;
}
...
void TAMC_GT911::writeByteData(uint16_t reg, uint8_t val) {
  G911.beginTransmission(addr);
  G911.write(highByte(reg));
  G911.write(lowByte(reg));
  G911.write(val);
  G911.endTransmission();
}
uint8_t TAMC_GT911::readByteData(uint16_t reg) {
  ...
  G911.write(lowByte(reg));
  G911.endTransmission();
  G911.requestFrom(addr, (uint8_t)1);
  x = G911.read();
  return x;
}
